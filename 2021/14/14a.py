...
# compute counts after 10 steps
ten_step_polymerize = np.linalg.matrix_power(polymerize, 10)
ten_step_polymer = np.matmul(ten_step_polymerize, polymer)
print(sum(ten_step_polymer))
...
letter_counts = [0]*26
for i, k in enumerate(sorted_keys):
    letter_counts[upper_char_to_num(k[0])] += ten_step_polymer[i]
    letter_counts[upper_char_to_num(k[1])] += ten_step_polymer[i]
