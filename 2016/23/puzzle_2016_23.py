...
TGL_MAP = {"tgl": "inc", "dec": "inc", "inc": "dec", "jnz": "cpy", "cpy": "jnz"}
...
def run_instructions(instructions, initial, output=False):
    ...
    while 0 <= index and index < len(instructions):
        ...
        if i[0] == "inc":
            ...
            tgl_index = index
            tgl_index += registers[i[2]] if i[1] else i[2]
            if tgl_index >= 0 and tgl_index < len(instructions):
                print(f"toggling at {tgl_index} the instruction {instructions[tgl_index][0]}")
                instructions[tgl_index][0] = TGL_MAP[instructions[tgl_index][0]]
        elif i[0] == "cpy":
            if i[3] is True:
...
def main_a(instructions):
    print(run_instructions(instructions, [7, 0, 0, 0]))
